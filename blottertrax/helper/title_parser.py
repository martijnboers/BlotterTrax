...
class TitleParser:
    ...
    @classmethod
    def get_artist_name_from_submission_title(self, post_title):
        # get main artist
        artist = None
        
        # let's start with a sample title "a-B (FEAT c) - -- - d [e/f](g)"
        artist = self.remove_prioritized_dash(post_title, 0)
        ...
        for feature in ['feat.', 'ft.', 'feature', 'featuring', ' feat ', '(feat ']:
            ...
            feature_artist = post_title[feat_index:].strip() # removes all before feat_index and strips, feature_artist = "c) - -- - d [e/f](g)"
            break
    ...
    @classmethod
    def get_song_name_from_submission_title(self, post_title, artist_name):
        ...
        # remove artist
        post_title = post_title.rsplit(artist_name[0].lower(), 1)[1] # "a-b (feat c) - -- - d [e/f](g)" -> " (feat c) - -- - d [e/f](g)"
        ...
        for feature in ['feat.', 'ft.', 'feature', 'featuring', ' feat ', '(feat ']:
            ...
            post_split = post_title.rsplit(feature, 1) # create a variable with all on both sides of featuring, because it may be before or after song.
            #post_split = [" ", "c) - -- - d [e/f](g)"]
            
            if post_split[0].strip() != "" and post_split[0].strip() not in left_tag: # decide if song to left or right of featuring. in our case " ".strip == ""
        ...
        post_title = self.remove_prioritized_dash(post_title, 1) # "- d [e/f](g)"
        ...
        # if first character is dash and split <= 2 chars, assume the formatting was bad and remove until first whitespace
